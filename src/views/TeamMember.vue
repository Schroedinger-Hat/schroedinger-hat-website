  ...
  <div class="event">
    <div class="container">
      <div class="content">
        ...
        <h4>
          <a class="external-link-color" target="_blank"
        ...
        <p><i>{{ $t(`events.${eventKey}.subtitle`) }}</i></p>
        <div class="description" v-html="$t(`events.${eventKey}.description`)"></div>
        <div class="cta">
          <a v-if="$t(`events.${eventKey}.signup-link`) !== ''"
            class="btn btn-primary" target="_blank"
            :href="$t(`events.${eventKey}.signup-link`)">{{ $t(`message.common.go-to-event`) }}</a>
          <a v-if="$t(`events.${eventKey}.cfp`) !== ''"
            class="btn btn-primary" target="_blank"
            :href="$t(`events.${eventKey}.cfp`)">{{ $t(`message.common.go-to-cfp`) }}</a>
          <a v-if="$t(`events.${eventKey}.donation`) !== ''"
            class="btn btn-primary" target="_blank"
            :href="$t(`events.${eventKey}.donation`)">{{ $t(`message.common.go-to-donation`) }}</a>
        </div>
        <div class="sponsors">
          <br />
          <h3>Sponsors</h3>
          ...
          <div v-if="$t(`events.${eventKey}.comunity-sponsors`) !== ''"
            class="sponsor-logos" v-html="$t(`events.${eventKey}.comunity-sponsors`)"></div>
        </div>
      </div>
    </div>
  </div>
