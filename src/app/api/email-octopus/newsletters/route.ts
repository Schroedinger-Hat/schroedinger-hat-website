...
export async function GET() {
  try {
    ...
    if (!apiKey || !listId) {
      return NextResponse.json({ error: "EmailOctopus configuration is missing" }, { status: 500 })
    }

    // First, get all recent campaigns
    const response = await fetch(
      `https://emailoctopus.com/api/1.6/campaigns?api_key=${apiKey}&limit=20&status=SENT`,
      {
        method: "GET",
        headers: {
    ...
    const filteredCampaigns = data.data.filter((campaign) => campaign.to.includes(listId)).slice(0, 6) // Limit to 5 for performance
    ...
    const newslettersWithContent: NewsletterContent[] = await Promise.all(
      filteredCampaigns.map(async (campaign) => {
        ...
        const contentResponse = await fetch(
          `https://emailoctopus.com/api/1.6/campaigns/${campaign.id}?api_key=${apiKey}`,
          {
            method: "GET",
            headers: {
              Accept: "application/json",
    ...
    return NextResponse.json({ error: "An error occurred while fetching newsletters" }, { status: 500 })
  }
}
